...
// older versions to match current functionality
  ...
  if($oldversion < 2005061700) {
						...
						if(!execute_sql("INSERT INTO `".$CFG->prefix."internalmail_block` VALUES ('3','search',200504200,0,0,1,0)")){
							return false;
						}
  ...
  if($oldversion < 2006032800) {
		...
		if($principals){
			...
			{	
				...
				$principal->course = 1;
				update_record($principal);
				$principal_boxes = get_records('internalmail_posts', 'parent', $principal->id);
				
				foreach ($principal_boxes as $principal_box)
				{	//les pestanyes de inbox, deleted, send i courses.
					$principal_box->course = 1;
					update_record($principal_box);
		...
		{	
			...
			$desti = make_upload_directory("$course/$CFG->moddata/internalmail/$cur_inter->id");	
									
			$newhash = md5_file ( $attachment->attachment);
