...
class block_internalmail_notify extends block_list {
	...
	if($this->content !== NULL) {
        ...
        $this->content->footer = "<br />";//get_string('courses_notify','internalmail');
	...
	foreach ($cursos as $curs){
		...
		if ($course->id == $curs->course) {
		    $this->content->items[]="<a href=\"{$interurl}view.php?a={$id->id}\"><span class=\"internalmail-notify-current-course\">{$name->fullname}</span>" .
			"&nbsp;<img src=\"{$CFG->wwwroot}/mod/internalmail/images/newmail.gif\" /></a>";
		} else {
		    $this->content->items[]="<a  href=\"{$interurl}view.php?a={$id->id}\"><span class=\"internalmail-notify-other-courses\">{$name->fullname}</span>" .
			"&nbsp;<img src=\"{$CFG->wwwroot}/mod/internalmail/images/newmail.gif\" /></a>";
		}
